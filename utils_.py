...
Notice:
    ...
    historical version from Face Identity Coding
"""
import torch
...
def pickle_dump_gzip(file_path, file):
    if not os.path.exists(file_path):
...
def pickle_dump_joblib(file_path, file, compress=0):
    if not os.path.exists(file_path):
        joblib.dump(file, file_path, compress=compress, protocol=-1)
    else:
        raise RuntimeWarning('[Codwarning] file already exists, continue will add extra data into that file')
...
def pickle_load(file_path):
    if os.path.exists(file_path):
        ...
        raise RuntimeError(f'[Coderror] can not find the file {file_path}')
        
def pickle_load_gzip(file_path):
    if os.path.exists(file_path):
...
def pickle_load_joblib(file_path, mmap_mode=None):
    if os.path.exists(file_path):
        loaded_file = joblib.load(file_path, mmap_mode=mmap_mode)
        return loaded_file
    else:
        raise RuntimeError('[Coderror] can not find the file')
