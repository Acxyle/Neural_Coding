...
def plot_filters(weights, cmap='seismic', normalize=True, cl_max=None):
    """
    ...
    nrows, ncols = gridshape(num_filters, tol=2.0)     # get the number of rows and columns in the grid
    outer_grid = gridspec.GridSpec(nrows, ncols)     # build the grid for all of the filters
    ...
    for w, og in zip(weights, outer_grid):
        ...
        # ----- [how this works? need to know the details of the algorithm]
        spatial, temporal = ft.decompose(w)     # get the spatial and temporal frame
        ...
        if cl_max is None:
            ax.imshow(spatial, interpolation='nearest', origin='lower', cmap=cmap, vmin=vmin, vmax=vmax)
        else:
            ax.imshow(spatial, interpolation='nearest', cmap=cmap, clim=colorlimit)
            
        fig.add_subplot(ax)
        ax.set_xticks(np.arange(spatial.shape[0]))
        ax.set_yticks(np.arange(spatial.shape[1]))

        ax = plt.Subplot(fig, inner_grid[1])
        ax.plot(temporal, 'k', lw=2)
        ax.plot(np.zeros((len(temporal),)), 'r', linestyle='--', lw=1)
        ax.grid()
        fig.add_subplot(ax)
...
def plot_spatial_grid(weights, cmap='seismic', normalize=True, cl_max=None):
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            if cl_max is None:
                ...
                ax.imshow(weights[row, col], interpolation='nearest', cmap=cmap, clim=colorlimit)
            plt.grid('off')
            plt.axis('off')
...
def get_sta(model, layer_id, samples=50000, batch_size=50, print_every=None, subunit_id=None):
    ...
    impulse = np.random.randn(2, 90, 90, 20).astype('uint8')
    ...
    for batch in range(int(np.ceil(samples/batch_size))):
        whitenoise = np.random.randn(batch_size, 90, 90, 20).astype('float32')
    ...
    # when the sta is of an affine layer
        ...
        sta = sta.reshape(true_response_shape[0], 20, 90, 90)
        return sta
    else:
